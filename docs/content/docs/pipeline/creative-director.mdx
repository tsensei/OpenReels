...
In local mode (`--local` or `--provider ollama`), the `ai_image` visual type is removed from the Zod schema at runtime. The LLM only sees `stock_image`, `stock_video`, and `text_card` as valid options. This is a schema-level constraint, not a prompt instruction, so the model cannot accidentally pick a visual type that requires a cloud image generation API. A `StubImageProvider` acts as a safety net in case anything slips through.
