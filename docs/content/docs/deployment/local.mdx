...
Edit `.env` with your API keys. At minimum you need one LLM key and one TTS key (or use `kokoro` for free local TTS). Or skip all paid keys and use `--local` with Ollama. See [API Keys](/docs/getting-started/api-keys) for details.
...
```bash
# Full local mode — no paid API keys (Ollama + Kokoro + bundled music)
pnpm start "topic" --local
...
# Choose providers
pnpm start "topic" --provider gemini --tts-provider kokoro --image-provider openai
...
# 3. Run with Ollama as LLM provider
pnpm start "topic" --provider ollama

# Or full local mode (Ollama + Kokoro TTS + bundled music)
pnpm start "topic" --local

# Use a different model
pnpm start "topic" --local --ollama-model gemma4:31b
```

OpenReels verifies that Ollama is running and the model is available before starting the pipeline.
