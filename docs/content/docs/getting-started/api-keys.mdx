...
You need at least **one LLM provider** and **one TTS provider** to generate a video. Or use `--local` for a zero-API-key setup with Ollama + Kokoro.
...
| | Ollama (free, local) | No key needed | [ollama.com/download](https://ollama.com/download). Use `--provider ollama` or `--local` |
...
| `--provider ollama` | Sets LLM to Ollama (local). No LLM API key needed. |
| `--provider local` | Sets LLM to Ollama and TTS to Kokoro. No LLM or TTS API key needed. |
| `--local` | Full local mode: Ollama + Kokoro + bundled music + stock visuals. No paid API keys for LLM, TTS, or music. Only needs a free Pexels key for stock footage. |
