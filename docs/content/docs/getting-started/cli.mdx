...
|---|---|---|
| `-p, --provider <name>` | LLM provider: `anthropic`, `openai`, `gemini`, `ollama`, `google`, `local` | `anthropic` |
...
- **`--provider local`** sets LLM to Ollama and TTS to Kokoro (free, local inference, no API key).
- **`--local`** sets everything for local mode: Ollama LLM + Kokoro TTS + bundled music + stock visuals + no stock verification + no AI video. No paid API keys needed.
...
### Full local mode (no paid API keys)

Run the entire pipeline on your own hardware. Only needs a free Pexels key for stock footage:

```bash
pnpm start "5 stoic lessons that changed my life" --local
```

### Free local TTS

Use Ollama for LLM and Kokoro for voiceover. Still needs an image API key:
