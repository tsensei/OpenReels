...
|-----------|-----------|---------|
| **LLM** | Anthropic Claude, OpenAI GPT, Google Gemini, **Ollama (local)** | `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GOOGLE_API_KEY`, none |
...
| `openai` | LLM only. |
| `gemini` | LLM only. |
| `ollama` | LLM only (local inference). No API key needed. |
| `google` | LLM (Gemini) + Image (Gemini Imagen) + TTS (Gemini TTS) + Video (Veo) + Music (Lyria). One `GOOGLE_API_KEY` for everything. |
| `local` | LLM (Ollama) + TTS (Kokoro, free local). No LLM or TTS API key needed. |
...
Minimum to run:
- **One LLM key** — `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `GOOGLE_API_KEY`. Or use `--provider ollama` / `--local` for free local inference (no key needed).
...
|------------------|----------------|-----------|---------|
| **LLM** | Ollama (free, local) | Gemini 2.5 Flash (~$0.001/run) | Anthropic Claude ($0.05-0.10/run) |
...
|------|-------------|---------|
| `--provider <name>` | LLM provider (`anthropic`, `openai`, `gemini`, `ollama`, `google`, `local`) | `anthropic` |
| `--local` | Full local mode: Ollama + Kokoro + bundled music + stock visuals | off |
